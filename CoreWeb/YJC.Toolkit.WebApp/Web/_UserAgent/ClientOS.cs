...
namespace YJC.Toolkit.Web
{
    ...
    {
        private static readonly Dictionary<string, string> fVersionMap = new Dictionary<string, string>{
            {"4.90","ME" },
            { "NT3.51","NT 3.11"},
            { "NT4.0","NT 4.0"},
            { "NT 5.0","2000"},
        ...
        {
            foreach (var matchItem in fMatchs)
            {
        ...
        private static readonly List<MatchExpression> fMatchs = new List<MatchExpression> {
            new MatchExpression {
            ...
            new MatchExpression {
                ...
                Action = (Match match, Object obj) => {
                    ...
                    if(current.Name.Length<match.Value.Length)
                    {
                        ...
                        current.Version = fVersionMap.Keys.Any(m => m == version) ? fVersionMap[version] : version;
                    }
                }
            },
            new MatchExpression {
