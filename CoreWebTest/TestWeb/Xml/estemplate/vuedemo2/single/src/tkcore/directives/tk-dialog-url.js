...
参数含义：
  ...
  title 是对话框的标题，
  height 为对话框的宽度。其中0为默认值，-1为最大屏幕高度，>0为实际像素点高度。
...
Vue.directive('tkDialogUrl', {
  ...
  componentUpdated: function(el, binding, vnode) {
    ...
    el.onclick = async function(e) {
      if (!url) {
        TkDebug.debug('tk-dialog-url.js', 'v-tk-dialog-url设置格式错误', false)
        return
      }
      
      vnode.context.dialogUrl = url
      vnode.context.dialogTitle = title
      vnode.context.dialogHeight = height
      vnode.context.dialogWidth = width
      vnode.context.dialogCloseOnClickModal = closeOnClick

      // TODO closeAction
      // vnode.context.dialogCloseAction = closeAction;

      if (url.startsWith('http')) { // start http or https
        vnode.context.dialogUseiFrame = true
      } else {
        vnode.context.dialogUseiFrame = false
        const components = await resolveComponents(
          vnode.context.$router.getMatchedComponents(url)
        )
        // console.log(components)
        if (components.length === 0) {
          TkDebug.debug('tk-dialog-url.js', '设置的url找不到路由', false)
          return
        } else if (components.length === 1) {
          // Vue.component('dialog-inner-form', components[0].default)
          TkDebug.debug('tk-dialog-url.js', '设置的url找不到路由', false)
          return
        } else {
          // 注意：这里根据加载机制，也许是Vue.component('dialog-inner-form', components[1].default)，暂未调查
          Vue.component('dialog-inner-form', components[1].default || components[1])
        }
      }
      vnode.context.dialogVisible = true
    }
  }
