...
(function ($) {
    ...
    // ----------------------------
    // 页面初始化
    Toolkit.page.isTopWin = ($('.tk-wrapper').length > 0) ? true : false; //判断是否为框架top页
    Toolkit.page.isMulTab = ($('ul#tkTabs').length > 0) ? true : false; //判断是否支持多Tab
    if (window != top) {
    ...
    // ----------------------------
    // URI相关封装
    Toolkit.page.getAbsoluteUrl = function (url) {
        if (url.startWith("/"))
            return url;
        var absolute = $("body").attr("data-webPath");
        return absolute + url;
    };

    Toolkit.page.getFullUrl = function (url) {
        var newUri = new URI(Toolkit.page.getAbsoluteUrl(url));
        var current = new URI(window.location.href);
        return newUri.absoluteTo(current).toString();
    };

    Toolkit.page.adjustUrl = function (params, delParamNames) {
        return Toolkit.page.addQueryString(document.location.href, params, delParamNames);
    };

    Toolkit.page.addQueryString = function (url, params, delParamNames) {
        var uri = new URI(url);
        var queryParams = uri.search(true);
        if (delParamNames) {
            if (typeof (delParamNames) === typeof (String)) {
                delete queryParams[delParamNames];
            } else if ($.isArray(delParamNames)) {
                for (var delParam in delParamNames) {
                    delete queryParams[delParam];
                }
            }
        }
    ...
    // 弹窗相关方法的二次封装
    // Toolkit.util.dialog 
    Toolkit.page.weixinClose = function () {
        if (typeof WeixinJSBridge != "undefined" && WeixinJSBridge.call) {
    ...
    Toolkit.page.prompt = function (message, onCallback, onCancel) {
        var TK = (this.isSubWin) ? top.Toolkit : Toolkit;
        TK.load('util.dialog', 'confirm', { message: message, callback: onCallback, cancel: onCancel });
    };
    ...
    Toolkit.page.dialog.alert = function (params) {
        Toolkit.load('util.dialog', 'alert', params);
    };
    ...
    Toolkit.page.dialog.prompt = function (params) {
        Toolkit.load('util.dialog', 'prompt', params);
    };
    ...
    Toolkit.page.main.initAccordion = function () {
        ...
        Toolkit.page.main.loadHTML(element, function () {
            ...
            if (element.find('.ui-treeview').length > 0) {
