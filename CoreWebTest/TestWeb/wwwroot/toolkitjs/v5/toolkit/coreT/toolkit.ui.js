...
(function ($) {
    ...
    Toolkit.ui.dataFormListActionExecute = function (elm) {
        ...
        elm.find('Button.ui-btn-checkall').click(function () {
            ...
            parent.find('thead input.e-checkall').prop('checked', true);
            parent.find('tbody.list input.row-index').bindToolkitUI('_checkAll', true);
            return false;
        });
        elm.find('Button.ui-btn-checknone').click(function () {
            var parent = $(this).closest('[id]');
            parent.find('thead input.e-checkall').prop('checked', false);
            parent.find('tbody.list input.row-index').bindToolkitUI('_checkAll', false);
            return false;
        });
        elm.find('Button.ui-btn-checkreverse').click(function () {
            var parent = $(this).closest('[id]');
            parent.find('tbody.list input.row-index').bindToolkitUI('_checkReverse');
            return false;
        });
        // 删除处理
    ...
    Toolkit.ui.linkChange = function (self, PData, CData) {
        var selectData = PData || $(self).data('addition');
        ...
        var optionData = CData || $(self).find('option:selected').data('addition');
        ...
        if (selectData && optionData) {
            ...
            for (var i in selectData) {
                for (var j in optionData) {
                    if (j == selectData[i].DecoderNickName) {
                        var _that = $(self),
                            panelid = $(self).attr('data-panelid');
                        if (panelid) {
                            let inputName = selectData[i].DataNickName, trIndex = $(self).attr('data-trindex');
                            $(`#${panelid}`).find('tr').eq(parseInt(trIndex)).find(`[name = ${inputName}]`).val(optionData[j]).attr("data-ss", optionData[j]);
                        } else {
                            $("#" + selectData[i].DataNickName).val(optionData[j]).attr("data-ss", optionData[j]);
                        }
                    }
                }
            }
    ...
    Toolkit.ui.dataLinkExecute = function (elm) {
        ...
        elm.find("select[data-addition]").change(function () {
            var $select = $(this);
            if ($select.parents('tr').length > 0 && !$select.attr('data-trindex')) {
                $select.attr({ 'data-trindex': $select.closest('.table-row')[0].rowIndex, 'data-panelid': $select.closest('.tk-datatable').attr('id') });
            }
            //控件data
            Toolkit.ui.linkChange(this);
        });
        elm.find('.panel-collapse-trigger').on('click', function (e) {
            $(e.currentTarget).siblings('.panel-collapse').collapse('toggle');
            $(e.currentTarget).children('.trigger-icon').toggleClass('rotate-180');
        });
        elm.find('.checkbox .checkbox-label').on('click', function (e) {
            input = $(e.currentTarget).siblings('input[type="checkbox"]').trigger('click');
        })
        elm.find('.radio .radio-label').on('click', function (e) {
            input = $(e.currentTarget).siblings('input[type="radio"]').trigger('click');
        })
        //elm.find("[data-control='EasySearch'] input[data-addition]").siblings('input').change(function () {
        //    //控件data
        //    Toolkit.ui.linkChange(this);
        //});
    };
    ...
    Toolkit.ui.easySearchDialog = function (input, data) {
        ...
        var url = Toolkit.page.addQueryString(data.url, { "InitValue": data.initValue, "RefValue": refString, "addition": Toolkit.page.currentEasySearchElement.attr('data-addition') });
    ...
    Toolkit.ui._onFileChange = function () {
        ...
        var maxsize = parent.data('maxsize');
