...
namespace YJC.Toolkit.JWT
{
    public interface IKeyManagement
    {
        ...
        /// <param name="header">
        /// JWT headers, dictionary can be mutated as part of call (e.g. keys added, e.t.c)
        /// </param>
        /// <returns>2 byte[] arrays: [0]=plain CEK, [1]=encrypted CEK</returns>
        byte[][] WrapNewKey(int cekSizeBits, object key, IDictionary<string, string> header);
        ...
        byte[] Unwrap(byte[] encryptedCek, object key, int cekSizeBits, IDictionary<string, string> header);
    }
}