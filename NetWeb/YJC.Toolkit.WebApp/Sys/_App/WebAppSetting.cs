...
namespace YJC.Toolkit.Sys
{
    public sealed class WebAppSetting : BaseAppSetting, ISupportDbContext
    {
        ...
        private readonly string fUploadTempVirtualPath;
        private readonly string fUploadVirtualPath;
        private readonly AbbreviateHandlerXml fAbbreviateConfig;
        ...
        internal WebAppSetting(string basePath, WebAppXml xml)
        {
            ...
            if (xml.Upload != null)
            {
                string baseUploadPath = Path.GetFullPath(Path.Combine(AppPath, ".."));
            ...
            fAbbreviateConfig = new AbbreviateHandlerXml();
            string abbrFile = Path.Combine(XmlPath, "Abbr.xml");
            if (File.Exists(abbrFile))
            {
                fAbbreviateConfig.ReadXmlFromFile(abbrFile);
            }
        ...
        public IConfigCreator<IPostObjectCreator> DefaultPostCreator { get; private set; }
        ...
        internal void Config(WebAppExtensionXml extXml)
        {
            ...
            if (DefaultPageMaker == null)
                DefaultPageMaker = new SourceOutputPageMakerConfig();
            if (DefaultRedirector == null)
                DefaultRedirector = new OutputRedirectorConfig();
            if (DefaultPostCreator == null)
                DefaultPostCreator = new JsonPostDataSetCreatorConfig();
        ...
        internal AbbreviateConfigItem GetAbbreivateConfig(string name)
        {
            return fAbbreviateConfig.FindItem(name);
        }
